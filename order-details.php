...
$email=$_POST['email'];
$ret = mysqli_query($con,"select t.email,t.id from (select usr.email,odrs.id from usuarios as usr join ordenes as odrs on usr.id=odrs.userId) as t where  t.email='$email' and (t.id='$orderid')");
$num=mysqli_num_rows($ret);
if($num>0)
{
$query=mysqli_query($con,"select productos.productImage1 as pimg1,productos.productName as pname,ordenes.productId as opid,ordenes.quantity as qty,productos.productPrice as pprice,ordenes.paymentMethod as paym,ordenes.orderDate as odate,ordenes.id as orderid from ordenes join productos on ordenes.productId=productos.id where ordenes.id='$orderid' and ordenes.paymentMethod is not null");
$cnt=1;
...
<?php $cnt=$cnt+1;} } else { ?>
				<tr><td colspan="8">El ID de pedido o el ID de correo electrónico registrado no son válidos</td></tr>
				<?php } ?>
			</tbody><!-- /tbody -->
		</table><!-- /table -->
